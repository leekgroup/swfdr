...
There are, in principle, many possible ways to use covariate variables when tackling the multiple hypothesis testing problem. This vignette does not attempt an unbiased exposition of the available methods; some of them have been summarized and benchmarked by @korthauer2018practical. Instead, this vignette provides a practical guide to using package `swfdr`, which implements algorithms for modeling the effect of covariates on false discovery rates. The theoretical aspects and modeling details are provided by @BocaEtAl2015 and @JagerEtAl2013, which themselves extend methods by @storey2002direct, and @storey2003statistical. In practice, these methods take in a set of p-values and output another set of quantities, q-values, that can be interpreted in the context of a multiple hypothesis analysis. 
...
For this vignette, we will also use Bioconductor package `qvalue`, @qvalue2018package. This is automatically installed in the procedure above. Thus, we can immediately load it. 
...
The table displays hit counts at various thresholds. To simplify the discussion, let's just look at the rows labeled 'p-value' and 'q-value' and the threshold of  0.05. (For a discussion of local FDR, see @qvalue2018package.) A helper function will be useful to extract the relevant data.
...
It is instructive to inspect the confusion matrix at various thresholds. 
...
## Local false discovery rates (argument `monotone`)

In addition to q-values, it is possible to compute local false discovery rates. The details of this calculation are explained in the `qvalue` package documentation, @qvalue2018package. The calculation can be extended to the context with covariates. However, because in `swfdr` each test instance is associated with its own $\pi_0$, there is one practical distinction. Whereas the `qvalue` package uses an argument `monotone` with a recommended value of `TRUE`, that argument is given a default value of `FALSE` in `swfdr`. This default value performs better in the case of the conceptual dataset `D2` in the vignette.  

Argument `monotone` is a new, i.e. expermental, feature in `lm_qvalue`. 
