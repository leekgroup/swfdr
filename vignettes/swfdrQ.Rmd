...
# this is an object used during development.
# To use the css object defined within the package, leaves mycss="swfdr"
# To use a custom plot style, replace mycss with that Rcss object
if (!exists("mycss")) {
  mycss="swfdr"
}
```
...
For this vignette, we will also use Bioconductor package `qvalue`, @qvalue2018package. This too can be installed via Bioconductor
...
Suppose we have measurements from two groups. To perform a statistical analysis, we would formulate a null hypothesis, compute a p-value against that hypothesis, and then apply some criteria to call the result as significant or not. The practical details may vary from application to application. For example, a common use case is to assume that measuremets come from distributions with equal variances, formulate the hypothesis that the distributions have equal means, and compute p-values using a t-test. An interpretation strategy might consist of calling a result a 'hit' or 'significant' if the p-value is below a threshold, often 0.05.
...
From this data, we can compute p-values for each rows in the dataset using a t-test. Because a similar calculation will be required again later, it is best to define a helper function.
...
The $\pi_0$ estimate for `D0` is unity, which is consistent with how we generated the dataset. For `D1`, $\pi_0$ is much lower. At `r round(D1q$pi0, 3)`, it is greater than the true value (zero; none of the instances in `D1` were generated in a way consistent with the null hypothesis). However, this estimate is nonetheless closer to the truth. The $\pi_0$ estimate for the combined dataset, `D2`, suggests that roughly half of the instances are consistent with the null hypothesis. 
...
We already computed q-values with the function `qvalue` in the previous section. We can display a summary for the `D0` dataset as follows. 

```{r}
summary(D0q)
```

The table displays hit counts at various thresholds. To simplify the discussion, let's just look at the rows labeled 'p-value' and 'q-value' and the threshold of  0.05. A helper function will be useful to extract a subset of the relevant results.
...
Let's move to the second dataset, in which all instances are inconsistent with the null hypothesis. 
...
Continuing the previous side note, how many hits would we have using a Bonferroni correction? The answer is `r hit_counts(D1q, 0.05/N)[1]` - thus leaving an even larger number of false negatives. This shows that procedures that use adjustments of p-values are quite distinct from q-values.
...
There are no positive hits - we have lost all signal. However, we know that calling hits based on q-values produced correct result based on `D0` and `D1` separately. Thus, it might be possible to improve the calculation for the joint dataset. This leads to using background knowledge to condition calculations of $\pi_0$ and q-values in the next section. 
...
In addition to a core set of data, many datasets come with additional descriptive variables. While these metadata might not enter in primary statistical analysis, they may nonetheless be valuable in a larger analysis pipeline. Package `swfdr` provides two functions, `lm_pi0` and `lm_qvalue`, that use linear models to condition estimates for $\pi_0$ and q-values.


## Conditioned pi0

Using the datasets and examples of the previous section, let's compute $\pi_0$ estimates for `D2` conditioned on the dataset of origin.
...
The top part reiterates the function call and the section on `lambda` reveals details how the results were estimated, see @storey2002direct and @storey2003statistical for details. The next section summarizes the covariates.
...
In contrast to the single $\pi_0$ estimate from function `qvalue`, we now have one estimate per instance in the dataset. That is because function `lm_pi0` uses logistic regression to estimate how the distribution of input p-values depends on the covariate, see @BocaEtAl2015 for details. The conditioned $\pi_0$ values range from `r round(min(D2_lm_pi0$pi0), 3)` to `r round(max(D2_lm_pi0$pi0), 3)`.

It is also possible to see a graphical summary of the conditioned $\pi_0$ estimates using a histogram.

```{r, echo=TRUE, eval=FALSE}
hist(D2_lm_pi0)
```
```{r D2_lm_pi0_hist, echo=FALSE, eval=TRUE, fig.width=2.5, fig.height=1.8}
hist(D2_lm_pi0, css=mycss, cssclass=c("ps8", "pdf"))
```

The histogram reveals that the distribution of $\pi_0$ estimates is bimodal with two very sharp peaks. The biomodal distribution matches the way the dataset was constructed, with one set of experiments consistent with the null hypothesis and another set with a strong signal. The red line indicates a $\pi_0$ estimate without conditioning on the covariate, and matches the result from `qvalue`.
...
Again, the output is a composite object and a summary is available.

```{r, eval=FALSE}
D2_lm_q
```

The majority of the output is analogous to the summary for `D2_lm_pi0`. The new section tabulates hit counts at various thresholds.

```{r, echo=FALSE, eval=TRUE}
print(D2_lm_q, components="hits")
```

We have `r hit_counts(D2_lm_q)[2]` hits based on q-values at a threshold of 0.05. We can further compute a confusion matrix.
...
This reveals there are `r D2conf[1,2]` false positives and `r D2conf[2,1]` false negatives. The analysis of the combined dataset with covariate thus reproduces the performance we obtained when we analyzed `D0` and `D1` separately.

We can also display a graphical summary of the results using the `plot` function.

```{r D2_lm_q_plot_code, echo=TRUE, eval=FALSE}
plot(D2_lm_q, var.x="p")
plot(D2_lm_q, var.x="q")
```
```{r D2_lm_q_plot, echo=FALSE, eval=TRUE, fig.width=5.0, fig.height=1.8}
oldpar = par()
par(mfrow=c(1,2))
plot(D2_lm_q, var.x="p", css=mycss, cssclass=c("pdf", "ps8"))
plot(D2_lm_q, var.x="q", css=mycss, cssclass=c("pdf", "ps8"))
par(mfrow=oldpar$mfrow)
```

The two views display the conditioned q-values against the input p-values (left) and against another set of q-values computed without conditioning on the covariate. Both views reveal two series of points - a consequence of the biomodal distribution of $\pi_0$ estimates we observed earlier. The vertical and horizontal guide lines offer clues which of the points would be called significant at a given threshold (by default, 0.05).
...
The new variable `truth.noisy` is a real number based on the previous `truth`, but carries considerable noise. We can repeat the calculations using this as a covariate.
...
In each case, the confusion matrix reveals a moderate or large number of true positives with a limited number of false positives. Thus, even in this noisy setting, the hit rate remains high and false discovery stays low.

We can also examine the result graphically, first through a histogram of the conditioned $\pi_0$.

```{r, echo=TRUE, eval=FALSE}
hist(D2_noisy_lm_q, css=mycss, cssclass=c("pdf", "ps8"))
```
```{r D2_noisy_hist, echo=FALSE, eval=TRUE, fig.width=2.5, fig.height=1.8}
oldpar = graphics::par()
par(mfrow=c(1,1))
hist(D2_noisy_lm_q, css=mycss, cssclass=c("pdf", "ps8"))
par(mfrow=oldpar$mfrow)
...
The distribution is broader than before, but it nonethless very different from the single point estimate that can be obtained without covariates.

Scatter plot compare the reported q-values with the original p-values and un-conditioned q-values.

```{r D2_noisy_plot_code, echo=TRUE, eval=FALSE}
plot(D2_noisy_lm_q, var.x="p")
plot(D2_noisy_lm_q, var.x="q")
```
```{r D2_noisy_plot, echo=FALSE, eval=TRUE, fig.width=5.0, fig.height=1.8}
oldpar = graphics::par()
par(mfrow=c(1,2))
plot(D2_noisy_lm_q, var.x="p", css=mycss, cssclass=c("pdf", "ps8"))
plot(D2_noisy_lm_q, var.x="q", css=mycss, cssclass=c("pdf", "ps8"))
par(mfrow=oldpar$mfrow)
```

The dashed lines, set at 0.05 by default, provide visual guides to separate points below the threshold. The plot on the right, in particular, reveals that modeling detects significant hits when none were called with unconditioned q-values.



\vspace*{0.5cm}

Although this last example introduced some noise, all calculations in this section are still based on synthetic data and are thus quite artificial. We have also considered just one covariate. Nonetheless, the examples are instructive because the analytical workflow and syntax also apply in more complex scenarios, as demonstrated in the next section.
...
In this vignette, we will use metadata columns `N` and `Freq_MAF_Hapmap`. The first is an integer field conveying the number of patients in the study for which the SNP was measured. This impacts on the power of the individual statistical tests. The second feature is the prevalence of the polymorphism in a background population. This impacts the likelihood that the SNP is associated with an unhealthy phenotype. 


## Conditioned pi0s with multiple covariates

The calculation for $\pi_0$ proceeds just like in the previous section. In this case, however, argument `X` should be a matrix or data frame containing all covariates of interest.
...
The structure the output is by now familiar. One of the sections confirms that the function used two variables in the calculation.

The final table summarizes the distribution of $\pi_0$ values. Recall that $\pi_0$ estimate the probability that a null hypothesis is true. Here, the estimates range from `r round(min(GWAS_lm_pi0$pi0), 4)` to `r max(GWAS_lm_pi0$pi0)`.

```{r GWAS_hist, echo=FALSE, eval=TRUE, fig.width=2.5, fig.height=1.8}
hist(GWAS_lm_pi0, css=mycss, cssclass=c("pdf", "ps8"))
```

The values are all fairly close to unity and suggest that the test instances are largely concordant with no association between SNPs and BMI. This is an expected property for a GWAS study. Nonetheless, individual cases diverge from the central estimate by up to 20%.
...
We can preview the contents of the output object by executing it in the console.

```{r, echo=TRUE, eval=FALSE}
GWAS_lm_qvalue
```

Most of the output is analogous to that of `GWAS_lm_pi0`. The new section is a summary of hits at various thresholds. 
...
In this calculation, `lm_qvalue` displays a warning that raises attention to the fact that a function meant to model covariates is used without any covariates at all. This message is useful to detect coding errors, e.g. unintentionally providing an empty object for modeling. Here, omitting the covariates was the intended action, so we can look past the warning and inspect the output. 

```{r, eval=TRUE, echo=FALSE}
print(GWAS_lm_qvalue_0, components=c("pi0", "hits"))
```

Although there are 50,000 estimates for pi0, all the estimates are equal. The hit table is slightly different than before. 
...
Interpreting statistical results can be challenging, especially when an analysis involves a large number of tests and p-values. When the aim is to call significant hits while controlling the rate of false discoveries, it can be useful to re-frame the interpretation in terms of q-values. The influencial package `qvalue` provides functions for this purpose that are applicable when all the statistical tests in an analysis are *a-priori* similar and can be weighted equally. In complex datasets, however, metadata may hold additional signals about the tested relationships. The `swfdr` package provides functions for using such metadata as covariates while controlling false discovery rates.

In this vignette, we saw how to install the software and walked through the analysis of small datasets. These examples, being synthetically generated, do not capture the nuances and complexities of a real-word analysis. Instead, the hope is that they capture the value of using q-values for controlling false discovery and the benefits of informative covariates. In particular, they demonstrate the utility of q-values for interpreting datasets with a high proportion of instances that are significant, as well as for modeling with covariates to identify hits when they are surrounded by data that is concordant with the null hypothesis. These are illustrative examples; further evaluations are described in the original articles, @BocaEtAl2015 and @boca2018direct, as well as in an independent review, @korthauer2018practical, which concluded that `swfdr` consistently performs well in a range of scenarios.
...
As hinted in one of the early sections, estimates for $\pi_0$ are computed from the distribution of the input p-values. Details for the algorithm are described e.g. in @storey2002direct and @storey2003statistical. Briefly, the calculations take a threshold, say $\lambda=0.05$, and evaluate the proportion of p-values below that threshold, and then repeat for several thresholds, say $\lambda=0.1, 0.15, \ldots$. By using such estimates together with a spline fit, the calculations arrive at a single estimate for $\pi_0$. Argument `lambda` provides the series of thresholds used in this process.
...
In many common applications, the default `lambda` vector will be appropriate and it is recommended to leave it unchanged. For small datasets, it may be advisable to provide a shorter vector, e.g. `seq(0.05, 0.95, 0.1)`.
...
Covariates should contain only numeric data; factors and categorical data are not supported.
...
Modeling the effect of covariates on $\pi_0$ is controlled via argument `type`, which has a default value of `'logistic'`. In this configuration, functions `lm_pi0` and `lm_qvalue` use logistic regression with one term for each given covariate. The other supported value for the `type` argument is `'linear'`, which uses a linear regression model instead. 
...
It is recommended to leave `type` at its default value. The argument is provided primarily for backward compatibility and as an inerface for exploring other modeling approaches.
